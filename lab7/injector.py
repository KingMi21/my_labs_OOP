...
class LifeStyle:
    PER_REQUEST = "PerRequest"  # Новый экземпляр при каждом запросе
    SCOPED = "Scoped"  # Один экземпляр в пределах области видимости
    SINGLETON = "Singleton"  # Единственный экземпляр на все время работы
...
class Injector:
    ...
    def register(self,
        ...
        if factory_method and class_type:
            raise ValueError("Нельзя указывать одновременно class_type и factory_method")
    ...
    def get_instance(self, interface_type: Type[T]) -> T:
        if interface_type not in self._registrations:
            raise ValueError(f"Не найдена регистрация для {interface_type.__name__}")
